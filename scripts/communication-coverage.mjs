...
const report = {
  ...
  cycle: 32,
  centroComunicacionComplete: true,
  ...
  coverage: {
    ...
    legacyDispatchTracking: 'trg_order_event_notify_customer + notify_riders + business_new_order via emit_communication_event',
    financeTemplates: 'communication_templates finance_* seeded',
    consentWeeklyDigest: 'get_admin_consent_weekly_digest + send-consent-weekly-digest',
    trackingPushUnified: 'enqueue_communication_push + claim_communication_retries',
    trackingAbVariants: 'communication_template_variants order_tracking_update A/B',
    legacyMigrationStats: 'get_admin_legacy_comm_migration_stats + legacy-migration.service.js',
    trackingPushDedup: 'record_order_location_ping only; no client send-push (cycle 19)',
    trackingOutboxRetired: 'tracking_push_outbox_archive + get_tracking_push_queue_stats (cycle 20)',
    retryProcessorUnified: 'process-comm-retries handles tracking push + all channels (cycle 21)',
    retryProcessorAdmin: 'get_comm_retry_processor_stats + retry-processor.service.js + AdminCommunicationsPanel',
    queueThresholdAlerts: 'communication_queue_thresholds + check_communication_queue_thresholds + queue-threshold.service.js',
    queueHealthDashboard: 'get_admin_queue_health + auto_resolve_queue_threshold_alerts + AdminCommunicationsPanel',
    queueHealthHistory: 'communication_queue_health_snapshots + record_queue_health_snapshot + queue-health.service.js',
    queueHealthExport: 'get_admin_queue_health_export + downloadQueueHealthCsv',
    queueHealthWeeklyReport: 'get_admin_queue_health_weekly_report + send-queue-health-weekly-report + queue-health-report.service.js',
    queueRecovery: 'reset_stale_communication_queue + requeue_failed_communications + queue-recovery.service.js',
    queueFailedArchive: 'communication_delivery_queue_archive + purge_failed_communications + get_admin_failed_queue_items',
    queueArchiveAutoPurge: 'auto_purge_failed_communications + get_admin_queue_archive_stats + check-comm-sla (cycle 30)',
    queueInWeeklyReport: 'get_admin_communication_weekly_report queue metrics + report.service.js + send-comm-weekly-report',
    moduleIntegration: 'emitCommEvent en account/client/discovery/info/support + get_admin_module_event_stats (cycle 31)',
    centroComunicacionClosure: 'get_admin_communication_closure_summary + closure.service.js + AdminCommunicationsPanel (cycle 32)',
  },
  pendingEvents,
  moduleHits,
