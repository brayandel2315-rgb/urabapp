-- 063: Republicar ofertas vencidas + TTL 2 min (estándar Rappi/Uber)
...
-- Extender TTL de ofertas a 2 minutos
...
BEGIN
  ...
  ELSIF v_order.order_type IN ('commerce', 'envio') OR v_order.order_type IS NULL THEN
    IF v_order.business_id IS NOT NULL AND v_order.status NOT IN ('accepted', 'preparing') THEN RETURN 0; END IF;
    IF v_order.business_id IS NULL AND v_order.status NOT IN ('pending', 'accepted', 'preparing') THEN RETURN 0; END IF;
  ...
  FOR v_driver IN
    ...
    WHERE d.is_online AND d.verification_status = 'approved' AND d.municipio = v_order.dest_municipio
